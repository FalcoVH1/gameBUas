...
void Surface::fPlot( float x, float y, Pixel c)
{
	if ((x >= 0) && (y >= 0) && (x < m_Width) && (y < m_Height)) {
		double xInt, yInt;
		double xDec = modf(x, &xInt);
		double yDec = modf(y, &yInt);
		//printf("%f\n", xInt);
		//printf("%f\n", yInt);
		double overlapAreaBL = 255 * (((xInt - (xInt - (1 - xDec))) * (yInt - (yInt - (1 - yDec))))),
		overlapAreaBR = 255 * (((xInt - (xInt - xDec)) * (yInt - (yInt - (1 - yDec))))),
		overlapAreaTL = 255 * (((xInt - (xInt - (1 - xDec))) * (yInt - (yInt - yDec)))),
		overlapAreaTR = 255 * (((xInt - (xInt - xDec))) * (yInt - (yInt - yDec)));

		//double sum = overlapAreaBL + overlapAreaBR + overlapAreaTL + overlapAreaTR;

		m_Buffer[(int)xInt + (int)yInt * m_Pitch] = (255 * (int)overlapAreaBL) / 100 + ((255 * (int)overlapAreaBL / 100) << 8) + ((255 * (int)overlapAreaBL / 100) << 16);
		m_Buffer[((int)xInt + 1) + (int)yInt * m_Pitch] = (255 * (int)overlapAreaBR) / 100 + ((255 * (int)overlapAreaBR / 100) << 8) + ((255 * (int)overlapAreaBR / 100) << 16);
		m_Buffer[(int)xInt + ((int)yInt + 1) * m_Pitch] = (255 * (int)overlapAreaTL) / 100 + ((255 * (int)overlapAreaTL / 100) << 8) + ((255 * (int)overlapAreaTL / 100) << 16);
		m_Buffer[((int)xInt + 1) + ((int)yInt + 1) * m_Pitch] = (255 * (int)overlapAreaTR) / 100 + ((255 * (int)overlapAreaTR / 100) << 8) + ((255 * (int)overlapAreaTR / 100) << 16);
	}
}
...
{
	//Allow me to boldly assume this is what was intended when the tutorial said "fix Surface::Bar". If it wasn't; Oops..

	assert(y1 < ScreenHeight && y2 < ScreenHeight);
